jQuery( function( $ ) {
    ...
    $( document ).ready( function() {
        ...
        $( addform ).find( "input[type='submit']" ).on( 'click', function( e ) {
            ...
            /*
            var r = new RegExp(/((?:(?:http?|ftp)[s]*:\/\/)?[a-z0-9-%\/\&=?\.]+\.[a-z]{2,4}\/?([^\s<>\#%"\,\{\}\\|\\\^\[\]`]+)?)/);
            // check for valid url
            $( '.ep-organizer-admin-social input[type="url"]' ).each(function() {
                
                let urlValue = $( this ).val();
                let key = $(this).data('key');
                if( urlValue && !is_valid_url(urlValue ) ) {
                    let invalidUrl = get_translation_string( 'invalid_url' );
                    if( $( this ).closest( '.ep-organizer-'+key ).find( '.ep-error-message' ).length < 1 ){
                        $( this ).closest( '.ep-organizer-'+key ).append('<div class="ep-error-message">'+invalidUrl+'</div>');
                    }
                    $( this ).focus();
                    formError = 1;
                    return false;
                } else{
                    $( this ).closest( '.ep-organizer-'+key ).find( '.ep-error-message' ).remove();
                }
            });
