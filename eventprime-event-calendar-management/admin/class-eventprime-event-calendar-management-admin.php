...
//		}
        ...
        if ($current_page == 'ep-settings' || $current_page == 'ep-bulk-emails') {
            ...
            wp_enqueue_script(
                    ...
                    array('jquery', 'jquery-ui-core', 'jquery-ui-tooltip', 'jquery-effects-highlight', 'jquery-ui-sortable', 'jquery-ui-datepicker'), $this->version
            );
            wp_enqueue_script('ep-admin-utility-script', plugin_dir_url(__FILE__) . 'js/ep-admin-common-utility.js', array('jquery', 'jquery-ui-tooltip', 'jquery-ui-dialog'), $this->version);
            wp_localize_script(
                    'ep-admin-utility-script',
        ...
        if ($current_page == 'bookings' || $current_page == 'booking_edit') {
            ...
            wp_enqueue_script(
                    ...
                    array('jquery', 'jquery-ui-datepicker'), $this->version
            );
    ...
    public function ep_save_event_meta_boxes($post_id, $wp_post) {
        ...
        //print_r($post);die;
        if (empty($post['ep_event_meta_nonce']) || !wp_verify_nonce(wp_unslash($post['ep_event_meta_nonce']), 'ep_save_event_data')) {
            return;
        }
    ...
    public function em_create_event_organizer_data($term_id) {
        ...
        if ($is_error) {
            ...
            return new WP_Error('invalid_data', __('Invalid Data.'));
...
//		wp_update_post( $performer_post );
    ...
    public function ep_booking_filters() {
        ...
        if ($typenow == 'em_booking') 
        {
            ...
            //print_r($events);die;
            //$events = $ep_functions->get_events_field_data(array('id', 'name'));
            $selected_event = '';
    ...
    public function ep_admin_menus() {
        ...
        // attendees list page
        add_submenu_page( "ep_hidden_menu", __( 'Attendees List', 'eventprime-event-calendar-management'), __('Attendees List', 'eventprime-event-calendar-management'), $ep_user_menus_caps, 'ep-event-attendees-list', array( $this, 'ep_show_event_attendees_list' ) );
    }
    ...
    public function ep_in_plugin_update_message( $args,$response ) 
    {
        ...
        if ( $show_additional_notice ) 
        {
                ...
                <span class="ep_plugin_upgrade_notice">
                    <span class=""> <?php printf( __( 'Another exciting EventPrime release is ready! Please check the change log and the release notes for version <strong>%s</strong> for detailed information before updating.', 'eventprime-event-calendar-management' ), $args['new_version'] ); ?></span>
                </span>
    ...
    public function ep_dismissible_notice() 
    {
        ...
        if($current_page===true || $pagenow === 'plugins.php')
        {
            ...
            <p><?php  _e( "Congratulations, EventPrime has been updated to v4! Please make sure all EventPrime extensions are up-to-date. Older incompatible extensions will be automatically deactivated. You can download the latest versions from <a href='https://theeventprime.com/checkout/order-history/' target='_blank'>here</a>.", 'eventprime-event-calendar-management' ); ?></p>
            </div>
            <?php
        }
    }
    ...
    public function ep_add_custom_view_link_to_admin_bar($wp_admin_bar) {
        ...
        if (isset($post) && in_array($post->post_type, $array) && $post->post_status == 'publish') {
            ...
            $wp_admin_bar->add_node(array(
                ...
                'title' => $title,
                'href'  => $custom_url,
                'meta'  => array('target' => '_blank', 'class' => 'custom-view-link')
            ));
        }
    }
